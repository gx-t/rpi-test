...
static int f_scan()
{
    nrf24l01_write_reg(0x00, 0x03); //power up, RX
    ...
    while(running) {
        ...
        while(running && line --) {
            for(ch = 0; running && ch < 128; ch ++) {
                nrf24l01_write_reg(0x05, ch); //set channel (data sheet page 54)
                gpioWrite(17, 1); //chip enable
                usleep(500); //data sheet p.20, f.3
                gpioWrite(17, 0); //chip disable
                uint8_t cd = 0;
                nrf24l01_read_reg(0x09, &cd); //carrier detect (data sheet page 55)
                if(ch && !(ch % 10))
                    printf("|");
                printf(cd & 1 ? "*" : " ");
                hist[ch] += (cd & 1);
...
static int f_carrier()
{
    ...
    nrf24l01_write_reg(0x00, 0x02); //power up, TX
    usleep(1500); //data sheet p.20, f.3
    nrf24l01_write_reg(0x05, (uint8_t)ch); //set channel (data sheet page 54)
    nrf24l01_write_reg(0x06, 0x90 | (power << 1)); //AN p.5. Bit 7 - "Cont wave" is not documented in datasheet.
    gpioWrite(17, 1); //chip enable
    while(running) {
        pause();
    }
...
static int spi_op(int (*f)())
{
    ...
    nrf24l01_print_regs();
    int res = f();
    nrf24l01_write_reg(0x00, 0x00); //power down
    nrf24l01_print_regs();
    spiClose(spi);
    gpioTerminate();
    return res;
